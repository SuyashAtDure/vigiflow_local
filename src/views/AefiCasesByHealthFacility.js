...
const createChartOptions = (title, data) => {
  ...
  chart: {
    ...
    yAxis: {
      ...
      tickInterval: 10, // Sets the interval between ticks on the y-axis
      min: 0, // Sets the minimum value of the y-axis
      max: 50, // Sets the maximum value of the y-axis
...
function AefiCasesByHealthFacility() {
  ...
  const getAllChartsData = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        setChartData(response.data[selectedCountry]["AEFI cases by Health Facility"]);
      }
    } catch (error) {
      setLoading(false);
  ...
  if (!chartData || Object.keys(chartData).length === 0) {
    return <HighchartsReact
      ...
      options={createChartOptions("AEFI cases by Health Facility", {})}
    />;
  }
  ...
  return (
    <HighchartsReact
      highcharts={Highcharts}
      options={createChartOptions("AEFI cases by Health Facility", chartData)}
    />
  );
}

export default AefiCasesByHealthFacility;
