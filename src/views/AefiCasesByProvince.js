...
const createChartOptions = (title, data) => {
  ...
  return {
    ...
    yAxis: {
      ...
      max: 70, // Sets the maximum value of the y-axis
...
function AefiCasesByProvince() {
  ...
  const getAllChartsData = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        setChartData(response.data[selectedCountry]["AEFI cases by Province"]);
      }
    } catch (error) {
      setLoading(false);
  ...
  if (!chartData || Object.keys(chartData).length === 0) {
    return <HighchartsReact
      ...
      options={createChartOptions("AEFI cases by Province", {})}
    />;
  }
  ...
  return (
    <HighchartsReact
      highcharts={Highcharts}
      options={createChartOptions("AEFI cases by Province", chartData)}
    />
  );
}

export default AefiCasesByProvince;
