...
function AefiCasesBySex() {
  ...
  const getAllChartsData = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        setChartData(response.data[selectedCountry]["AEFI cases by Sex"]);
      }
    } catch (error) {
      setLoading(false);
  ...
  if (!chartData || Object.keys(chartData).length === 0) {
    return <HighchartsReact
      ...
      options={createChartOptions("AEFI cases by Sex", {})}
    />;
  }
  ...
  return (
    <HighchartsReact
      highcharts={Highcharts}
      options={createChartOptions("AEFI cases by Sex", chartData)}
    />
  );
}

export default AefiCasesBySex;
