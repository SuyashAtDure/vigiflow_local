...
function AefiCasesByVaccine() {
  ...
  const getAllChartsData = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        setChartData(response.data[selectedCountry]["AEFI cases by Vaccine"]);
      }
    } catch (error) {
      setLoading(false);
  ...
  if (!chartData || Object.keys(chartData).length === 0) {
    return <HighchartsReact
      ...
      options={createChartOptions("AEFI cases by Vaccine", {})}
    />;
  }
  ...
  return (
    <HighchartsReact
      highcharts={Highcharts}
      options={createChartOptions("AEFI cases by Vaccine", chartData)}
    />
  );
}

export default AefiCasesByVaccine;
