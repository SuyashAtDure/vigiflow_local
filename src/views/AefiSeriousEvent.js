...
function AefiSeriousEvent() {
  ...
  const getAllChartsData = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        setChartData(response.data[selectedCountry]["AEFI SERIOUS Event"]);
      }
    } catch (error) {
      setLoading(false);
  ...
  if (!chartData || Object.keys(chartData).length === 0) {
    return <HighchartsReact
      ...
      options={createChartOptions("AEFI Serious Event", {})}
    />;
  }
  ...
  return (
    <HighchartsReact
      highcharts={Highcharts}
      options={createChartOptions("AEFI Serious Event", chartData)}
    />
  );
}

export default AefiSeriousEvent;
